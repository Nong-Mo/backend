...
class QueryProcessor:
    ...
    async def process_query(self, user_id: str, query: str, new_chat: bool = False, save_to_history: bool = True):
        ...
        try:
            ...
            # 4. STORY
                ...
                try:
                    ...
                    story_prompt = f"""
                        ...
                        4. 같은 인용구가 반복되어도 반드시 **"인용 내용"**의 형식으로 처리
                        ...
                        - 인용된 내용은 반드시 **"인용된 내용"** 으로 처리해야 합니다
                        - 영감의 내용이 자연스럽게 이야기에 녹아들어야 합니다
                        - 이야기는 자연스럽게 연결되어야 합니다
                        - 이야기의 맥락이 자연스럽게 연결되어야 합니다
                        - 아래의 출력 형식은 반드시 따라야 합니다
                        - 제목에 1막, 2막, 3막은 포함하지 않습니다

                        [문장 구성 원칙]
                        1. 인용구 도입 전:
                        - 상황 설명으로 인용구와 연결되는 맥락 제시
                        - 개발 경험과 연결되는 복선 배치
                        ...
                        * # (1,2,3)막: (내용이 연관되고 핵심 인용구를 활용한 제목)
                        * ### (본문 내용, 각 문장에 적용)
                        - 각 막의 내용은 다음을 포함:
                        * 1막: 정글 입성, 첫 도전, 어려움
                        ...
                        * 3막: 최종 프로젝트 발표 준비와 발표. 그리고 모두와의 이별
                        ...
                        - {contents_text}의 핵심 문구는 **"핵심 문구"** 의 형식으로 강조
                        ...
                        - 본문은 ### 로 작성
                        - 내적 독백은 *이탤릭체*
                        - {contents_text} 직접 인용구는 **"인용 내용"** 의 형식으로 반드시 처리
                        - 절대 인용구를 '인용 내용'으로 처리하지 않습니다. 작은 따옴표로 처리하지 않습니다.
                        ...
                        * 멘토님
                        ...
                        [출력 형식]
                        # (해당 막의 내용에서 핵심을 추출하면서 핵심 인용구를 활용한 제목)
                        ### (내용)
            ...
            [시스템 규칙]
            1. "A2D 서비스 사용 방법을 알려줘"라고 말하면 다른 말 붙이지 말고 무조건

            "안녕하세요, {nickname}님!"
            "저는 A2D 서비스의 AI 어시스턴트입니다."
            ""
            "아날로그 데이터를 사진으로 찍거나 업로드해서 원하는 보관함에 저장한 후에 자유롭게 활용하세요!"
            "보관함에 저장된 데이터를 조합하여 이야기로 창작해 보는 건 어떠신가요?"

            각각의 문장들은 띄어서 출력하세요. 

            2. "스토리 창작은 어떻게 하면 돼?"라고 말하면 다른 말 붙이지 말고 무조건
            "안녕하세요, {nickname}님!"
            "저는 A2D 서비스의 AI 어시스턴트입니다."
            ""
            "이야기를 만들어줘, 라고 얘기하시면 됩니다."
            "{nickname}님의 보관함에 있는 파일들을 조합하여 새로운 이야기를 만들고 있어요. 지금은 크래프톤 정글의 이야기로 녹여내고 있지만 앞으로 더 발전시킬 예정이니 잘 부탁드립니다!"

            각각의 문장들은 띄어서 출력하세요.
            위의 말을 보낸 이후에는 사용자 메시지에 대해 자유롭게 대답하세요.

            3. "A2D 서비스는 누가 개발했어?"라고 말하면 다른 말 붙이지 말고 무조건
            "안녕하세요, {nickname}님!"
            "저는 A2D 서비스의 AI 어시스턴트입니다."
            ""
            "A2D는 크래프톤 정글 7기의 농모 팀이 개발했습니다."
            "🌴 프론트엔드 개발자 권한비, 남서하, 류병현"
            "🌴 백엔드 개발자 김동현, 최재혁"
            "총 다섯 명의 정글러들이 A2D에 참여했어요."
            "한 달간 여정의 결과물을 자유롭게 즐겨보세요!"

            각각의 문장들은 띄어서 출력하세요.
            위의 말을 보낸 이후에는 사용자 메시지에 대해 자유롭게 대답하세요.

            4. 1번, 2번, 3번에 지정된 응답과 다른 응답이 도착하면 처음에는 무조건
            "안녕하세요, {nickname}님!"
            "저는 A2D 서비스의 AI 어시스턴트입니다."
            ""
